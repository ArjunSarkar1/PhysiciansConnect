...
class InvoiceExportUtilTest {
    ...
    @Test
    void testExportInvoice_txtBranch_and_cancel() throws Exception {
        // TXT branch, approve and cancel
        try (MockedStatic<JOptionPane> paneMock = mockStatic(JOptionPane.class, CALLS_REAL_METHODS);
             MockedStatic<JFileChooser> chooserMock = mockStatic(JFileChooser.class, CALLS_REAL_METHODS)) {
            ...
            when(chooser.showSaveDialog(any())).thenReturn(JFileChooser.APPROVE_OPTION);
            when(chooser.getSelectedFile()).thenReturn(tempFile);

            chooserMock.when(() -> new JFileChooser()).thenReturn(chooser);

            try (MockedStatic<InvoiceExportUtil> exportUtilMock = mockStatic(InvoiceExportUtil.class, CALLS_REAL_METHODS)) {
                exportUtilMock.when(() -> InvoiceExportUtilTestHelper.showOpenDialog(any(), any(), any())).then(inv -> null);

                InvoiceExportUtil.exportInvoice(null, invoice, "2024-01-01 10:00", payments);

                // Cancel TXT
                when(chooser.showSaveDialog(any())).thenReturn(JFileChooser.CANCEL_OPTION);
                InvoiceExportUtil.exportInvoice(null, invoice, "2024-01-01 10:00", payments);
            }
        }
    }
    ...
    @Test
    void testExportInvoice_pdfBranch_and_cancel() throws Exception {
        // PDF branch, approve and cancel
        try (MockedStatic<JOptionPane> paneMock = mockStatic(JOptionPane.class, CALLS_REAL_METHODS);
             MockedStatic<JFileChooser> chooserMock = mockStatic(JFileChooser.class, CALLS_REAL_METHODS);
            ...
            doNothing().when(job).print(any(PrintRequestAttributeSet.class));
