...
class WeeklyAvailabilityPanelTest {
    ...
    void testPanelLoadsWeekDataFromService() throws SQLException {
        ...
        Map<?, ?> loaded = (Map<?, ?>) getField(panel, "weekData");
    ...
    void testPanelFallsBackToAllFreeOnServiceException() throws SQLException {
        ...
        for (Object slots : loaded.values()) {
            assertEquals(16, ((List<?>) slots).size());
    ...
    void testClickOnFreeSlotShowsAddDialogAndPrefills() throws SQLException {
        ...
        try (MockedStatic<JOptionPane> mockedPane = mockStatic(JOptionPane.class);
            ...
            MouseEvent click = new MouseEvent(panel, MouseEvent.MOUSE_CLICKED, System.currentTimeMillis(), 0, x, y, 1, false);
            for (var l : panel.getMouseListeners()) l.mouseClicked(click);
    ...
    void testClickOnBookedSlotWithNoAppointmentShowsErrorDialog() throws SQLException {
        ...
        try (MockedStatic<JOptionPane> mockedPane = mockStatic(JOptionPane.class)) {
            ...
            mockedPane.verify(() -> JOptionPane.showMessageDialog(
                    any(), eq(UIConfig.ERROR_APPOINTMENT_NOT_FOUND), eq(UIConfig.ERROR_DIALOG_TITLE), eq(JOptionPane.ERROR_MESSAGE)));
        }
    }
    ...
    void testClickOnTimeLabelOrHeaderDoesNothing() throws SQLException {
        ...
        int y = 50;
        MouseEvent click1 = new MouseEvent(panel, MouseEvent.MOUSE_CLICKED, System.currentTimeMillis(), 0, x, y, 1, false);

        // Click in header row
        int x2 = 100;
        int y2 = 10; // < HEADER_HEIGHT
        MouseEvent click2 = new MouseEvent(panel, MouseEvent.MOUSE_CLICKED, System.currentTimeMillis(), 0, x2, y2, 1, false);

        for (var l : panel.getMouseListeners()) {
            l.mouseClicked(click1);
            l.mouseClicked(click2);
        }
        // No dialogs or panels should be shown, so nothing to assert, but no exceptions should occur
    }
