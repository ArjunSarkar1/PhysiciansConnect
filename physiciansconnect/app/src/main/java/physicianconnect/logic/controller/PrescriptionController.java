...
public class PrescriptionController {
    ...
    public void createPrescription(
        ...
        // We'll treat “start date” as today’s date
        LocalDate today = LocalDate.now();
        PrescriptionValidator.validateStartDate(today);
        // Duration isn’t entered by the UI, so we skip validateDuration here

        // 2) Build the timestamp strings
        String createdAt = LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);

        // 3) Construct a Prescription domain object
        //    (ID of 0 → let persistence layer assign if it auto‐increments)
        Prescription presc = new Prescription(
                0,                      // id (auto‐generated by DB/persistence)
                physicianId,            // who wrote it
                patientName,            // patient’s name
                medicationName,         // med name
                defaultMedicationDosage,// default dosage from Medication
                dosage,                 // actual dosage instructions
